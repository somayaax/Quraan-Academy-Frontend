...
export class ListRecordedCourseComponent implements OnInit {
  ...
  constructor(private recordedCourse : RecordedCoursesService, private toastr: ToastrService, private category : RecordedCourseCategoryService, private dialog: MatDialog) {
  ...
  getRecordedCourses(): void {
    const params = {
      page: this.page,
      category: this.selectedCategory !== 'None' ? this.selectedCategory  : null
    };
    this.recordedCourse.getRecordeCourses(params).subscribe({
      next: (data) => {
        this.recordedCourses = data.map((course: recordedCourseElement, index : number) => ({
          ...course,
          id: index + 1
        }))
      },
  ...
  deleteRecordedCourse(id: any) {
    this.recordedCourse.deleterecordedCourses(id).subscribe({
      next: (data) => {
        ...
        this.toastr.success(`Recorded course deleted successfully`,'Success');
      },
      error : (error : any) => {
        let {error : {message}}  = error;
        if(!message) message = error.error.error;
